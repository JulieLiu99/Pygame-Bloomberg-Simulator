...
def writeAccount(order):
    ...
    account_l = [account_d]
    portfoliopage.write_to_csv(account_l, 'account.csv')
...
#    if order.status == True:
    if order.order_type == 'Buy':
        for i in d:
            if i['Symbol'].lower == order.symbol:
                i['Shares'] = int(i['Shares']) + order.shares
                i['Cost'] += (','+ str(order.price))
                i['Type'] = 'Buy'
                break
            else:
                new_stock = {}
    ...
    if order.order_type == 'Sell':
        for i in d:
            if i['Symbol'].lower == order.symbol:
                ...
                new_stock['Cost'] = order.cost # bUG
                new_stock['Price'] = 0 # set as 0 for now, this should be updated with real price when displayed
                new_stock['Type'] = order.type
                d.append(new_stock)
    if order.order_type == 'Sell Short':
        pass
    if order.order_type == 'Buy to Cover':
        pass
    portfoliopage.write_to_csv(d,'portfolio.csv')
